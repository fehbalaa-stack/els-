...
function Megtalaltam() {
  ...
  useEffect(() => {
    const fetchChildData = async () => {
      try {
        const res = await axios.get(`https://gyerek-tracker-backend.onrender.com/api/profiles/public/${id}`);
  ...
  const sendLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      try {
        ...
        await axios.post(`https://gyerek-tracker-backend.onrender.com/api/profiles/public/location/${id}`, {
          lat: latitude,
          lng: longitude
        });
