...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      ...
      try {
        const res = await axios.get('http://10.199.99.94:5000api/profiles', { headers: { 'x-auth-token': token } });
  ...
  const onSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await axios.post('http://10.199.99.94:5000/api/profiles', formData, { headers: { 'x-auth-token': token } });
  ...
  const printQR = (profile) => {
    const link = `http://10.199.99.94:3000/megtalaltam/${profile._id}`;
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
        {profiles.map(profile => {
          const qrLink = `https://kh6hb7cz-3000.euw.devtunnels.ms/${profile._id}`;
          return (
